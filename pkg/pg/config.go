...
type Config struct {
	...
	AllowDDLCollection     bool   `mapstructure:"allow_ddl_collection"`
}
...
func ConfigFromViper(key *string) (Config, error) {
	...
	dbtuneConfig.BindEnv("allow_ddl_collection", "DBT_POSTGRESQL_ALLOW_DDL_COLLECTION")

	// Set defaults
	dbtuneConfig.SetDefault("include_queries", true)
	dbtuneConfig.SetDefault("maximum_query_text_length", 50_000)
	dbtuneConfig.SetDefault("allow_ddl_collection", false)
