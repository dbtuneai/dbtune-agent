...
SELECT md5(
    coalesce((
        SELECT string_agg(c.relname || ':' || a.attname || ':' || t.typname, ',' ORDER BY c.relname, a.attname)
    ...
    coalesce((
        SELECT string_agg(indexdef, ',' ORDER BY schemaname, tablename, indexname)
...
func CollectSchemaSnapshot(pool *pgxpool.Pool, allowDDLCollection bool, lastSchemaHash string, logger *logrus.Logger) (*agent.SchemaSnapshot, error) {
	...
	if allowDDLCollection && snapshot.SchemaHash != lastSchemaHash {
		...
		tables, indexes, err := collectDDLDetails(pool, ctx, logger)
		if err != nil {
			logger.Warnf("Failed to collect DDL details, skipping: %v", err)
			// Keep the old hash so the next tick retries DDL collection
			snapshot.SchemaHash = lastSchemaHash
		} else {
...
func collectDDLDetails(pool *pgxpool.Pool, ctx context.Context, logger *logrus.Logger) ([]agent.SchemaColumn, []agent.SchemaIndex, error) {
	rows, err := utils.QueryWithPrefix(pool, ctx, schemaTablesQuery, agent.MaxDDLTableColumns)
	if err != nil {
		return nil, nil, fmt.Errorf("schema tables query failed: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&col.TableID, &col.TableName, &col.ColumnName, &col.DataType); err != nil {
			return nil, nil, fmt.Errorf("schema tables scan failed: %w", err)
		}
		tables = append(tables, col)
	}
	if err := rows.Err(); err != nil {
		return nil, nil, fmt.Errorf("schema tables iteration failed: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("schema indexes query failed: %w", err)
	}
	defer idxRows.Close()
	...
	for idxRows.Next() {
		...
		if err := idxRows.Scan(&idx.IndexID, &idx.IndexDefinition); err != nil {
			return nil, nil, fmt.Errorf("schema indexes scan failed: %w", err)
		}
		indexes = append(indexes, idx)
	}
	if err := idxRows.Err(); err != nil {
		return nil, nil, fmt.Errorf("schema indexes iteration failed: %w", err)
	}
