...
//
//   - Struct reflection (the expensive part) happens once at init.
//   - Column matching happens once per distinct column layout, then is cached.
//   - If PostgreSQL is upgraded and SELECT * returns new columns, the new
//     layout is a cache miss, a new mapping is computed and cached, and the
//     new columns are silently skipped (or matched if you've added the field).
//   - If you connect to a fleet of mixed PG versions, each version's column
//     layout gets its own cached mapping. Both work correctly.
//
// Scanner is safe for concurrent use.
//
// Usage:
//
//	// At package init — all reflection happens here, once.
//	var pgStatScanner = pgxutil.NewScanner[PgStatDatabase]()
//
//	// At query time — zero reflection, cheap map lookup after first row per column layout.
...
// Scan implements pgx.RowToFunc[T]. Pass it directly to pgx.CollectRows.
//
// It is fully lax in both directions:
//   - Row columns with no matching struct field → silently skipped
//   - Struct fields with no matching row column → zero value
//
// All type conversion is delegated to row.Scan, which uses pgx's full
// pgtype.Map / PlanScan system (OID-aware, format-aware, all registered codecs).
...
// using a cached value if available. The cache key is derived from the
// column names in the result set, so different PG versions (which return
// different columns from SELECT *) each get their own mapping.
...
// colKey builds a cache key from the column names. Two result sets with
// identical column names (in order) share a mapping. This is cheap: just
// a single string join, and the result set from a given PG version + query
// is always the same column set.
func colKey(fldDescs []pgconn.FieldDescription) string {
	// Pre-size: average column name ~15 chars + separator.
